...
document.write('<tr><th align="center" colspan="4" >'+str_HeadTxt+' : '+ userName[permissionsUser] +'</th></tr>');
...
document.write('<tr><th id="top">' + str_Input + '</th><th id="top">' + str_PCF_Output + '<th id="top">' + str_PCA_Output + '</th><th id="top">' + str_PWMs + '</th></tr>');
...
for(i=1;i<=PCA.length;i++){	  
	if(PCA_description[i] != null || cb_SuperVisor==1){ 
		document.write('<tr>');
		document.write('<td align="center">'+i+'</td>')
		document.write('<td align="center">');
		if(PCA_description[i]==null){PCA_description[i]="";}
		document.write(PCA_description[i]);
		document.write('</td>');
		document.write('<td align="center">O<input type="checkbox" name="pReadPermission" value="'+i+'" ');
		if( pcaReadPermission[i-1] == 1){document.write('checked ');}
		document.write('onchange="changed(this)">');
		document.write('<input type="checkbox" name="pWritePermission" value="'+i+'" ');
		if( pcaWritePermission[i-1] == 1){document.write('checked ');}
		document.write('onchange="changed(this)">I</td></tr>');	 
	} 
}
document.write('</table></td><td><table id="t02" align="center">');
