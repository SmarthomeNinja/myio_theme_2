...
function setupCardReorder() {
  ...
  function checkSwap(ghostY) {
    if (!draggingCard || !ghost || isAnimating || Date.now() - lastSwapTime < 160) return;
    ...
    for (const c of cards) {
      ...
      const overlap = !(gRect.bottom < r.top || gRect.top > r.bottom);
      if (!overlap) continue;

      const overlapTop = Math.max(gRect.top, r.top);
      const overlapBottom = Math.min(gRect.bottom, r.bottom);
      const overlapH = Math.max(0, overlapBottom - overlapTop);

      if (overlapH >= 18) {
        const overTop = Math.max(0, gRect.bottom - r.top);
        const overBottom = Math.max(0, r.bottom - gRect.top);
        const before = (overTop >= overBottom);

        performSwapCard(draggingCard, c, before);
        lastSwapTime = Date.now();
        break;
      }
    }
  ...
  function performSwapCard(dragCard, targetCard, before) {
    if (isAnimating) return;
    isAnimating = true;
    ...
    const withoutDrag = all.filter(x => x !== dragCard);
    const tIndex = withoutDrag.indexOf(targetCard);

    const newOrder = [...withoutDrag];
    if (before) newOrder.splice(tIndex, 0, dragCard);
    else newOrder.splice(tIndex + 1, 0, dragCard);
    ...
    // újrarendezés DOM-ban
    newOrder.forEach(c => draggingGrid.appendChild(c));
    ...
    all.forEach(c => {
      ...
      if (dx || dy) {
        ...
        c.style.zIndex = (c === dragCard) ? "1000" : "1";
        c.offsetHeight;
