...
(function() {
  // Async script loader to avoid CORS issues with XHR and ensure dependencies load before use
  ...
  function loadScript(src) {
    return new Promise((resolve, reject) => {
      ...
      s.crossOrigin = 'anonymous'; // classic scripts don't require CORS, but set explicitly
      s.onload = () => {
        console.log('✓ Betöltve:', src);
        resolve();
      };
      s.onerror = (e) => {
        console.warn('✗ Nem sikerült betölteni (script):', src, e);
        reject(e);
      };
...
(function () {
    ...
    // Wait for dependency loader if present, otherwise run immediately
    if (window.myioDepsReady && typeof window.myioDepsReady.then === 'function') {
      window.myioDepsReady.then(run).catch(() => {
        // If deps failed to load, avoid errors by not running
        console.warn('Dependencies failed to load, renderers will not initialize.');
      });
    } else {
