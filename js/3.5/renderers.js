...
(function () {
  ...
  function renderZones(root) {
    const zones = window.myioStorage.loadZones();
    if (!zones.length) return;

    const cardFactories = window.myioCards.cardFactories; // Map object

    // Iterate over zones
    zones.forEach(zone => {
      // Collect cards for this zone
      const cardIdsInZone = [];

      // Strategy: iterate all known card factories (available cards) and check if they are assigned to this zone
      // This is slightly inefficient but safe. 
      // Alternatively, iterating assignments and checking if factory exists.

      // Let's use getCardFactory(id) for ids known to have assignments? 
      // No, we don't have a list of all available card IDs easily unless we iterate the factories.
      // But storage has assignments.

      // Better: Iterate all card factories.
      for (const [id, factory] of cardFactories.entries()) {
        const assignedZones = window.myioStorage.getCardZones(id);
        if (assignedZones.includes(zone.id)) {
          cardIdsInZone.push(id);
        }
      }

      if (cardIdsInZone.length === 0) return;

      const sectionKey = `myio.section.zone.${zone.id}`;
      const { section, grid } = makeSection(zone.name, '', sectionKey, false);

      cardIdsInZone.forEach(id => {
        try {
          if (hasCardFactory(id)) {
            grid.append(getCardFactory(id)());
          }
        } catch (e) { console.error(`Error rendering card ${id} in zone ${zone.name}:`, e); }
      });
  ...
  window.myioRenderers = {
    renderSensors, renderSwitches, renderPCA, renderFET, renderRelays, renderFavorites, renderZones
  };
})();
