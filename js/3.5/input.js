...
document.write(str_Tsens+'</th><th id="top"><button name="ReInit" value="1" onclick="changed(this)" align="left">'+str_ReInit+'</button></th></tr>');
document.write('<tr><th>EEprom ID</th><th>'+str_Name+'</th><th>Celsius</th></tr>');  
if (!fullZeroArray(thermo_eepromIndex)) {
	// Create an array of objects to sort by thermo_description
	let thermoData = [];
	for (i = 0; i < thermo_eepromIndex.length; i++) {
		if (thermo_eepromIndex[i] != 0) {
			thermoData.push({
				index: thermo_eepromIndex[i],
				description: thermo_description[thermo_eepromIndex[i]] || "",
				temp: thermo_temps[i]
			});
		}
	}

	// Sort the array alphabetically by description
	thermoData.sort((a, b) => a.description.localeCompare(b.description));

	// Generate the table rows
	for (let data of thermoData) {
		document.write('<tr><td align="center">' + data.index + '</td>');
		document.write('<td align="center">');
		document.write('<input type="text" size="25" maxlength="25" placeholder="-" ');
		document.write('name="t_descr*' + data.index + '" value="' + data.description + '" onchange="changed(this)"></td>');
		document.write('<td align="center">' + (data.temp / 100));
...
for(i=1;i<=switchEnabled.length;i++){
	...
	for(k=0;k<3;k++){
		...
		document.write('<button class="green_button" name="s_'+HPRwrite(k)+'" value="'+i+'" onclick="changed(this,this.name,1,true)" >'+HPRstring(k)+'</button>');
		...
		if (HPRevent(k,i-1) <3025 && HPRevent(k,i-1)>2000){document.write(' selected');}
		document.write('>'+str_PCA+'</option>');
		document.write('<option value="3"');
		if (HPRevent(k,i-1) <5025 && HPRevent(k,i-1)>4000){document.write(' selected');}
		...
		if (HPRevent(k,i-1) <1800 && HPRevent(k,i-1)>1699){document.write(' selected');}
		document.write('>' + str_LoPCA + '</option>');
		if (VersionCheck("3.7.1")) {
			document.write('<option value="15"');
			if (HPRevent(k,i-1) <1900 && HPRevent(k,i-1)>1799){document.write(' selected');}
