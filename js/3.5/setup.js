...
if (VersionCheck("3.11.1")){
	B_var("mDNS Host Name");
	document.write('http://'+mdns+'.local</td></tr>');
}
document.write('');
B_var(str_IPAddress);	
for(i=0;i<4;i++){
...
if(VersionCheck("3.7.4")){
	B_var("Longitude");
	document.write('<input type="number" style="width: 6em"  min="0" maxlength="10" name="longitude" value="' + longitude + '" onchange="changed(this)">');
	B_var("Latitude");
	document.write('<input type="number" style="width: 6em"  min="0" maxlength="10" name="latitude" value="' + latitude + '" onchange="changed(this)">');
}
...
if (VersionCheck("3.7.1")) {
	B_var(str_SDMLog);
	document.write('<input type="checkbox" name="SDMLogger" value="'+SDMLogger+'" ');
	if( SDMLogger == 1){document.write('checked ');}
	document.write('onchange="changed(this)"></td>'); B_End();
}
...
if (VersionCheck("3.11")) {
	document.write('<tr><td>BroadLink</td><td><input type="checkbox" name="BroadLink" value="'+_broadLink+'" ');
	if (_broadLink == 1) { document.write('checked '); } document.write('onchange="changed(this)">');
	document.write('<button onClick="window.location.href=\'/broadlink\'">Setup BroadLink</button>');
	document.write('</td ></tr > ');
}
...
if (VersionCheck("3.11.1")) {
	B_var(str_SubnetMask);	
	for(i=0;i<4;i++){
		document.write('<input type="number" style="width: 3em" min="0" max="255" maxlength="3" name="subnet*'+i+'" value="'+subnet[i]+'" onchange="changed(this)">');
		if(i<3){document.write(' . ');}
	}B_End();
}
B_var(str_HitTL);
...
if (VersionCheck("3.7.1")) {
	B_var(str_SDMTimer);
	document.write('<input type="number" style="width: 2em"  min="0" max="32000" maxlength="6" name="SDMTimer" value="' + SDMTimer + '" onchange="changed(this)"> sec'); B_End();
}
B_var(str_TempTimer);
...
B_var(str_EthInit);
document.write('<input type="number" style="width: 3em"  min="0" maxlength="6" name="ethernetTimer" value="'+ethernetInitTimer+'" onchange="changed(this)">sec');B_End();
...
document.write('<tr><td><button name="SensorBackup" value="1" onclick="changed(this,this.name,1,0,1)" >'+str_SensorBackup+'</button></td><td>'+str_SensorBackupHelp+'</td><tr>');
document.write('<tr><td><button name="SensorRestore" value="1" onclick="changed(this,this.name,1,0,1)" >'+str_SensorRestore+'</button></td><td>'+str_SensorRestoreHelp+'</td></tr>');
