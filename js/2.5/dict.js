...
speechDictionaryDefault = JSON.parse(speechDictionaryTXT); 
...
for (each in speechDictionaryDefault) {
	if (speechDictionary[each] == undefined) {
		speechDictionary[each] = speechDictionaryDefault[each];
	}
}
...
//Places
document.write('<table style="width:auto! important; background:transparent;"><tr><td>');
...
for (place in placeDictionary["places"]) {
	document.write('<tr style="outline: thin solid"><td align="center">' + place + '</td>');
...
document.write('</div>'); //places rawBox

//document.write('<div class="rawBox">');
//listener name
//document.write('<table style="float:right; width:auto! important; background:transparent;"><tr><td>');
document.write('<div class="rawBox" style=" float:right ! important">');
...
//document.write('</div>'); //listener & endword rawBox
//Verbs 
...
for (verb in speechDictionary["verbs"]) {
	document.write('<tr style="outline: thin solid" ><td align="center">' + verb + '</td>');
...
//document.write('<talbe style="width:auto! important; background:transparent;"><tr><td>');
	...
	if (Object.keys(subjectDictionary[subjectType]).length > 1) {
		...
		for (verb in subjectDictionary[subjectType]["verbs"]) {
			...
			for (word in subjectDictionary[subjectType]["verbs"][verb]) {
				...
				document.write('<td>');
				document.write('<input name="SubjectType*' + subjectType + '" id="SubjectTypeWord*' + word + '" alt="' + verb + '" type = "text" size = "15" maxlength = "20" placeholder = "+" ');
		...
		for (element in subjectDictionary[subjectType]) {
			if (element != "verbs") {
				document.write('<table id = "t02" style="outline: thin solid"><tr><td>');
				document.write('<table><tr><th id="top" colspan="100%">' + element + ' - ' + FindDescription(subjectType, element) + '</th></tr>');
				document.write('<tr>');
				//Subject Words
...
function AddDictionary(_obj, _force) {
	if (_obj.name.startsWith("listener")) {
		...
		if (!_exist || _force) {
			...
			var cell0 = row.insertCell(0);
			cell0.style.textAlign = "center";
			cell0.innerHTML = '<input type = "text" size = "9" maxlength = "20" placeholder = "+" name="Conjunction*' + __verb + '" alt="' + __conjunction + '" id="' + _newConjunctionID + '" value="' + _obj.value + '" onchange="ChangeDictionary(this);">';
		...
		if (!_exist || _force) {
			...
			var cell0 = row.insertCell(0);
			cell0.style.textAlign = "center";
			cell0.innerHTML = '<input type = "text" size="15" maxlength = "20" placeholder = "+" name="Verb*' + __verb + '" id="' + _newVerbID + '" value="' + _obj.value + '" onchange="ChangeDictionary(this);">';
		...
		if (!_exist || _force) {
			...
			var cell0 = row.insertCell(0);
			cell0.style.textAlign = "center";
			cell0.innerHTML = '<input type = "text" size="15" maxlength = "20" placeholder = "+" name="PlaceWord" alt="' + __place + '" id="' + _newPlaceID + '" value="' + _obj.value + '" onchange="ChangeDictionary(this);">';
		...
		if (!_exist || _force) {
			...
			var cell0 = row.insertCell(0);
			cell0.style.textAlign = "center";
			cell0.innerHTML = '<input type = "text" size="15" maxlength = "20" placeholder = "+" name="SubjectWord*'+_newSubjectWordID+'" alt="' + __type + '" id="SubjectElement*' + __element + '" value="' + _obj.value + '" onchange="ChangeDictionary(this);">';
